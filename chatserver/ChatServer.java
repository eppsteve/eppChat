...
public class ChatServer
{
	...
	//List of all client handling threads in the server.Synchronized
	Vector handlers;
	...
	public class ChatHandler implements Runnable
	{
		...
		protected boolean handShake()
		{
			try
			{
				...
				//Add client to list and send to it the complete
				//client list
			...
			try
			{
				...
				while ( true )
				{
					try
					{
						...
						else if( clientMsg instanceof FileTransferMessage )
						{
						    ((ChatHandler)(handlers.elementAt(((FileTransferMessage)clientMsg).recieverId))).out.writeObject(clientMsg);
						}
						...
						else if( clientMsg instanceof ChangeNick )
						{
							System.out.println(((ChangeNick)clientMsg).userID);
							window.clientList.setElementAt(((ChangeNick)clientMsg).newName, ((ChangeNick)clientMsg).userID);														
							broadcast((ChangeNick)clientMsg);				
						}
