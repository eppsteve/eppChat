...
public class ChatClient
{
			...
			while( true )
			{
				if( running )
				{
					...
					try
					{
						...
						else if( serverMsg instanceof ChangeNick )
						{
							clientList.setElementAt(((ChangeNick)serverMsg).newName, ((ChangeNick)serverMsg).userID);														
						}
						
						else if( serverMsg instanceof UpdateList )
						{
							//If information is about new user logging in
						...
						else if ( serverMsg instanceof FileTransferMessage )
                        {
                            if ( serverMsg instanceof FileTransferRequest )
                                receiveFile( (FileTransferRequest) serverMsg );
                            else
                                fileTransferHandler.setTransferMessage( (FileTransferMessage) serverMsg );
                        }
	...
	//Transmit a request to change nickname
	void changeNickName(String name)
	{
		ChangeNick nick = new ChangeNick();
		nick.newName = name;
		nick.oldName = "hi";
		nick.userID = clientID;
	...
	void sendFile( int receiverId, File fileToSend )
    {
        if( fileToSend.length() == 0 )
        {
            JOptionPane.showMessageDialog( this.window,"File size is 0(zero) bytes, file will not be sent" );
            return;
        }
        
        if ( (receiverId != clientID) && ((fileTransferHandler == null) || (fileTransferHandler.getCurrentMode() == FileTransferHandler.IDLE)) )
        {
            fileTransferHandler = new FileTransferHandler( clientID,receiverId,this );
            fileTransferHandler.sendFile( fileToSend );
        }
        else
        {
            if( receiverId == clientID )
                JOptionPane.showMessageDialog( this.window,"File cannot be sent to yourself" );
            else
            JOptionPane.showMessageDialog( this.window,"File cannot be transferred at this time" );
        }
    }

    /**
     * @param request
     *            The <code>FileTransferRequest</code> that the client
     *            received
     */
     
     
    void receiveFile( FileTransferRequest request )
    {
        if ( (fileTransferHandler == null) || (fileTransferHandler.getCurrentMode() == FileTransferHandler.IDLE) )
        {
            int choice = JOptionPane.showConfirmDialog( this.window, window.getFriendName( request.senderId ) + " is sending you " + request.getFileName() + ", \nAccept Transfer?","File Transfer Request",JOptionPane.OK_CANCEL_OPTION,JOptionPane.QUESTION_MESSAGE );
            fileTransferHandler = new FileTransferHandler( clientID,request.senderId,this );

            if ( choice == JOptionPane.OK_OPTION )
            {
                JFileChooser saveDialog = new JFileChooser();
                saveDialog.setFileSelectionMode( JFileChooser.FILES_ONLY );
                saveDialog.setSelectedFile( new File(saveDialog.getCurrentDirectory()+"\\"+request.getFileName()) );
                //saveDialog.setCurrentDirectory( new File(saveDialog.getCurrentDirectory()+request.getFileName()) );
                choice = saveDialog.showSaveDialog( this.window );

                if ( choice == JFileChooser.APPROVE_OPTION )
                    fileTransferHandler.receiveFile( saveDialog.getSelectedFile(),request.getFileSize(),true );
                else
	...
	synchronized void sendObject( Object o )
    {
        try
        {
            out.writeObject( o );
